...
HandGesturePipeline::HandGesturePipeline()
{
    ...
    // Detection pre/post
    m_detect_preprocess = new dl::image::ImagePreprocessor(
        m_detect_model, {0, 0, 0}, {1, 1, 1});
    m_detect_preprocess->enable_letterbox({114, 114, 114});

    m_detect_postprocess = new dl::detect::ESPDetPostProcessor(
        m_detect_model, m_detect_preprocess, DETECT_SCORE_THR, DETECT_NMS_THR,
        100, DETECT_STAGES);
    ...
    m_cls_preprocess = new dl::image::ImagePreprocessor(
        m_cls_model, {123.675f, 116.28f, 103.53f},
        {58.395f, 57.12f, 57.375f});

    m_cls_postprocess = new dl::cls::HandGestureClsPostprocessor(
        m_cls_model, CLS_TOP_K, CLS_SCORE_THR, true, "output");
